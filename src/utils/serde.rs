...
pub fn deserialize_f64_opt_from_str<'de, D>(deserializer: D) -> Result<Option<f64>, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        let f = if s.is_empty() {None} else {
            let r = f64::from_str(&s).map_err(serde::de::Error::custom)?;
            Some(r)
        };
        Ok(f)
    }
