use anyhow::Context;
use polars::{export::regex::Regex, prelude::{DataFrame, NamedFrom}, series::Series};
use duct::cmd;
...
fn main() -> anyhow::Result<()> {
    ...
    let status = parse_immotalctl_status(&output)?;
    ...
    send_mail(format!("Bot Report {hostname}"), body)?;
    Ok(())
}

fn parse_immotalctl_status(output: &str) -> anyhow::Result<DataFrame> {
    let ansi_color = Regex::new(r"\x1B\[([0-9]{1,2}(;[0-9]{1,2})*)?m")?;
    let ansi_red = Regex::new(r"\x1B\[0;31m")?;
    let is_up = output.lines().skip(1).map(|line| !ansi_red.is_match(line));
    let output = ansi_color.replace_all(output, "");
    let immortalctl_line_re = Regex::new(r"^(\d+)\s+([\ddhms\.]+)\s+(\w+)\s+(.+)$").unwrap();
    let mut ret = vec![vec![];4];
    for (line, up) in output.lines().skip(1).zip(is_up) {
